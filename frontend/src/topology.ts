...
export function buildTopology(model:ResourceModel,overview:Overview):Topology {
  ...
  if(model.type==='application'){const app:RawResource=applications.find(item=>item.name===model.name&&item.namespace===model.namespace)??{id:model.id,kind:'Application',name:model.name,namespace:model.namespace,item:null,labels:{},annotations:{},owners:[]};add(app,'controller',model.status);for(const root of roots){add(root,'controller');edges.push(edge(app,root))}}
